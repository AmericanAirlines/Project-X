...
const MockRepositoryResults: MockRepositoryDetails[] = [
    {node_id: "VeryGreatRepo", url: "www.github.com/AmericanAirlines/VeryLargePlane", name: "VeryLargePlane", stargazer_count: 12345},
    {node_id: "EvenBetterRepo", url: "www.github.com/AmericanAirlines/EvenBiggerPlane", name: "EvenBiggerPlane", stargazer_count: 54321}
];
...
describe("Repository POST route", () => {
    ...
    it("Successful fetch", async () => {
        ...
        expect(handler.entityManager.findOne).toHaveBeenCalledWith(Repository, {nodeID: MockRepositoryResults[0].node_id})
        expect(handler.entityManager.persist).toHaveBeenCalledWith(new Repository({nodeID: MockRepositoryResults[0].node_id}))

        expect(handler.entityManager.findOne).toHaveBeenCalledWith(Repository, {nodeID: MockRepositoryResults[1].node_id})
        expect(handler.entityManager.persist).toHaveBeenCalledWith(new Repository({nodeID: MockRepositoryResults[1].node_id}))
        
        expect(handler.entityManager.findOne).toHaveBeenCalledTimes(2);
        expect(handler.entityManager.persist).toHaveBeenCalledTimes(2);
