...
import { Heading, HStack, Spacer, useTheme, VStack, Text, Box, Button, ButtonGroup, FormControl, FormLabel, FormHelperText, Input } from '@chakra-ui/react';
  ...
  if(editToggle)
      ...
      <Box border="1px" borderColor="gray.200" boxShadow="base" p={3}>
        <form onSubmit={formik.handleSubmit}>
          <FormControl id="editUser">
            ...
            <Input id="pronouns" name="pronouns" type="text" value={formik.values.pronouns} onChange={formik.handleChange} />

            <FormLabel htmlFor="schoolName">School Name</FormLabel>
            <Input id="schoolName" name="schoolName" type="text" value={formik.values.schoolName} onChange={formik.handleChange} />
          </FormControl>
          <ButtonGroup>
    ...
    return (
      <Box border="1px" borderColor="gray.200" boxShadow="base" p={3}>
        ...
        <Button colorScheme="blue" onClick={() => setEditToggle(!editToggle)}>Edit</Button>
      </Box>
    );
};
