...
describe('web /invalidEmail', () => {
  it('renders page and makes sure buttons work as intended', async () => {
    ...
    expect(
      screen.queryByText("Try again once you've verified your student email address 👇"),
    ).toBeVisible();
    expect(screen.queryByText('Login with GitHub')).toBeVisible();
    expect(screen.queryByText('Login with GitHub')).toHaveAttribute(
      'href',
      '/api/auth/github/login',
    );
    expect(
      screen.queryByText('Click this button if you want to go back to the homepage 👇'),
    ).toBeVisible();
